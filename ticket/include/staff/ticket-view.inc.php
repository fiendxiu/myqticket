            ...
            <tr>
                ...
                <div>
                    ...
                    <span style="display:inline-block;padding:0 1em" class="faded title"><?php
                        echo Format::truncate($entry['title'], 300); ?></span>
                    </span>
...
?>
            ...
            <tr>
                <td>
                    <label><strong><?php echo '结案原因'; ?>:</strong></label>
                </td>
                <td>
                    <select name="close_reason_id">
                        <option>
<?php
$conn = mysql_connect("localhost", "root", "both-win");
mysql_select_db("ticket",$conn);
mysql_query("SET names UTF8");
$check_query = mysql_query("select close_reason from ost_ticket where ticket_id=${_GET['id']}");
echo mysql_fetch_array($check_query)[0];
mysql_close($conn);
?>
                        </option>
                        <option>客户需求</option>
                        ...
                        <option>dns问题</option>
                        <option>断线问题</option>
                        <option>voip问题</option>
                        <option>email 问题</option>
                        <option>bgp 问题</option>
                        <option>骨干问题</option>
                        <option>线路品质问题</option>
                        <option>内网问题</option>
                        <option>CE设备问题</option>
                        <option>流量满载</option>
                    </select>
                <script type="text/javascript">
                    function message()
                    {
                        <?php
                            $call_name=$thisstaff->getFirstName();
                            $call_ext=$thisstaff->phone_ext;
                            $call_ticketNum=$ticket->getNumber();
                            $mCallInfo=$call_ticketNum."-".$call_name;
                        ?>
                        call_info=document.getElementsByClassName("thread-body")[0].innerHTML.split('<br>')[4];
                        call_phone=call_info.split('-')[1];
                        var mCallPhone=prompt(
"当前用户："+"<?php echo $call_name; ?>\n\n"+
"分机号："+"<?php echo $call_ext; ?>\n\n"+
"客户号码："+call_phone+"\n\n"+
"当前ticket num："+"<?php echo $call_ticketNum; ?>\n\n"+
call_info+"\n\n\n请输入要拨打的客户电话：",
call_phone
);
                        if (mCallPhone!=null && mCallPhone!="") {
                            mCallUrl="http://192.168.38.254:8011/?phone="+mCallPhone+"&extension=<?php echo $call_ext ?>"+"&ticketInfo=<?php echo $mCallInfo ?>";
                            call_window=window.open(mCallUrl,'','height=100,width=100,scrollbars=yes,status=yes');
                            sleep(100);
                            call_window.close();
                        }
                    }
...
function sleep(d){
  for(var t = Date.now();Date.now() - t <= d;);
                    ...
                    <input type='button' onclick='message();' style='margin-left:200px' value='CALL'>
                </td>
            </tr>
            <tr>
                <td width="120">
