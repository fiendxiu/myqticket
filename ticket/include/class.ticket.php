...
class Ticket {
    ...
    function onResponse($response, $options=array()) {
        $cr=$options["close_reason"];
        db_query('UPDATE '.TICKET_TABLE.' SET isanswered=1, lastresponse=NOW(), updated=NOW(), close_reason="'.$cr.'" WHERE ticket_id='.db_input($this->getId()));
        $this->reload();
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function postReply($vars, &$errors, $alert=true, $claim=true) {
        ...
        $this->onResponse($response, array('assignee' => $assignee, 'close_reason' => $vars['close_reason_id'])); //do house cleaning..
