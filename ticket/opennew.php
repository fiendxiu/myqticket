...
$lineid = $_GET['lineid'];
preg_match('/\d+[A-Z][a-z]?/',$lineid,$match);
$siteid = $match[0];
...
$message="-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-<br />线路ID: $lineid<br />WANIP: $linewanip<br />合同名: $contractname <br />系统中的联络人资料:$sitecontact-$sitephone<br />报障人：  报障来电：  <br />-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-<br />";
...
if($ticket
    ...
    require(CLIENTINC_DIR.'opennew.inc.php');
}
